...
type Settings = { pattern: string; cell: number; coverage: number; lift: number; contrast: number; texture: number; fade: number; start: number; brightness: number; color: string; background: string; middle:string; spacing:number; stroke:number; rotation:number; sharpness:number; fadeEnd:number; fadeDirection:string; vignette:number; grain:number };
const DEFAULTS: Settings = { pattern: 'tonal', cell: 4, coverage: 50, lift: 32, contrast: 105, texture: 90, fade: 92, start: 15, brightness: 100, color: '#8a76e9', background: '#050508', middle:'#3e2f84', spacing:100, stroke:30, rotation:0, sharpness:30, fadeEnd:94, fadeDirection:'down', vignette:0, grain:0 };
const SHAPES:[string,string][] = [['tonal','十字层次（参考）'],['bayer','方点抖色（旧版）'],['cross','十字 ＋'],['x','交叉 ×'],['circle','圆点 ●'],['square','方块 ■'],['diamond','菱形 ◆'],['lines','平行线 ━']];
const PRESETS = [
...
export default function Home() {
  ...
  const geometric=!['tonal','bayer'].includes(settings.pattern);
  ...
  useEffect(()=>{
    const worker=new Worker('/processor.js?v=3');workerRef.current=worker;
  ...
  return <main onDragOver={e=>{e.preventDefault();if(e.dataTransfer.types.includes('Files'))setDragging(true);}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget as Node))setDragging(false);}} onDrop={e=>{e.preventDefault();setDragging(false);if(e.dataTransfer.files[0])void load(e.dataTransfer.files[0]);}}>
    ...
    <div className="workspace"><aside className="settings-panel"><div className="panel-title"><h1>把明暗，变成网点。</h1><p>紫黑色调 · 有序抖色 · 氛围渐暗</p></div>
      ...
      <section className="control-section"><div className="section-heading"><h2>02 <span>网点与层次</span></h2><SlidersHorizontal size={14}/></div><div className="pattern-control"><label>网点形状</label><Choice label="网点形状" value={settings.pattern} onChange={v=>update('pattern',v)} items={SHAPES}/><p>{geometric?'符号大小随图片明暗变化':'随明暗形成方点、十字与方孔'}</p></div><Range label="网点大小" value={settings.cell} min={1} max={12} unit="" onChange={v=>update('cell',v)}/><Range label="网点覆盖" value={settings.coverage} min={20} max={85} onChange={v=>update('coverage',v)}/><Range label="网点强度" value={settings.texture} onChange={v=>update('texture',v)}/><Range label="暗部细节" value={settings.lift} max={65} onChange={v=>update('lift',v)}/><details className="advanced"><summary>排列与细节<ChevronDown size={15}/></summary>{geometric&&<Range label="网点间距" value={settings.spacing} min={60} max={240} onChange={v=>update('spacing',v)}/>}<Range label="网点旋转" value={settings.rotation} min={-90} max={90} unit="°" onChange={v=>update('rotation',v)}/>{['cross','x','lines'].includes(settings.pattern)&&<Range label="笔画粗细" value={settings.stroke} min={10} max={90} onChange={v=>update('stroke',v)}/>}<Range label="细节增强" value={settings.sharpness} onChange={v=>update('sharpness',v)}/></details></section>
      <section className="control-section"><div className="section-heading"><h2>03 <span>色彩与氛围</span></h2></div><div className="color-row">{([['color','网点色'],['middle','中间色'],['background','暗部色']] as const).map(([k,t])=><label className="color-control" key={k}><input type="color" value={settings[k]} onChange={e=>update(k,e.target.value)} aria-label={t}/><span>{t}<small>{settings[k].toUpperCase()}</small></span></label>)}</div><div className="pattern-control fade-direction"><label>渐暗方向</label><Choice label="渐暗方向" value={settings.fadeDirection} onChange={v=>update('fadeDirection',v)} items={[["down","向下"],["up","向上"],["left","向左"],["right","向右"]]}/></div><Range label="渐暗强度" value={settings.fade} onChange={v=>update('fade',v)}/><Range label="渐暗起点" value={settings.start} max={settings.fadeEnd-1} onChange={v=>update('start',v)}/><Range label="渐暗终点" value={settings.fadeEnd} min={settings.start+1} max={100} onChange={v=>update('fadeEnd',v)}/><details className="advanced"><summary>明暗、暗角与颗粒<ChevronDown size={15}/></summary><Range label="整体亮度" value={settings.brightness} min={50} max={160} onChange={v=>update('brightness',v)}/><Range label="对比度" value={settings.contrast} min={60} max={160} onChange={v=>update('contrast',v)}/><Range label="四周暗角" value={settings.vignette} onChange={v=>update('vignette',v)}/><Range label="胶片颗粒" value={settings.grain} max={40} onChange={v=>update('grain',v)}/></details></section><button className="reset-button" onClick={()=>{setSettings({...DEFAULTS});setPreset('参考紫黑');}}><RotateCcw size={14}/>恢复参考效果</button>
      ...
      <div className={`preview-stage ${zoom?'zoomed':''}`}><div className="stage-corner top-left"/><div className="stage-corner bottom-right"/><div className={`image-frame ${view==='compare'?'comparing':''}`} onPointerDown={view==='compare'?comparisonDrag.down:undefined} onPointerMove={view==='compare'?comparisonDrag.move:undefined} onPointerUp={comparisonDrag.end} onPointerCancel={comparisonDrag.end} onLostPointerCapture={comparisonDrag.lost} style={{aspectRatio:source?`${source.width}/${source.height}`:'1.13','--ratio':source?source.width/source.height:1.13} as React.CSSProperties}><canvas ref={canvasRef} className="result-canvas" aria-label="处理后的紫黑网点图片"/>{source&&view!=='effect'&&<img draggable={false} className="original-image" src={source.url} alt="导入的原始图片" style={{clipPath:view==='compare'?`inset(0 ${100-split}% 0 0)`:undefined}}/>}{view==='compare'&&<><div className="compare-line" style={{left:`${split}%`}}><button type="button" className="compare-handle" role="slider" aria-label="拖动原图与效果分界线" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(split)} aria-valuetext={`原图占 ${Math.round(split)}%`} aria-orientation="horizontal" onKeyDown={e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End'].includes(e.key)){e.preventDefault();setSplit(v=>splitByKey(e.key,v,e.shiftKey));}}}>‹ ›</button></div><span className="image-label original-label">原图</span><span className="image-label effect-label">效果</span></>}{!source&&<div className="image-loading"><LoaderCircle className="spin" size={24}/><span>正在准备示例图片</span></div>}</div></div>
      {view==='compare'&&<div className="compare-control"><span>原图</span><Slider className="comparison-range" aria-label="原图与效果对比分界线" min={0} max={100} value={[split]} onValueChange={v=>setSplit(Array.isArray(v)?v[0]:v)}/><span>效果</span></div>}

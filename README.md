...
- **基础编辑**：曝光、对比、饱和度、色温、清晰度；常用或自定义画布比例、裁切、留边；透明贴图和多行文字的位置、缩放、旋转、描边与透明度。
- **顺序叠加**：每层可启用、停用、复制、删除、重排和调整强度。文字、贴图也参与顺序处理，海报可继续套入音乐卡片。
- **历史**：不设固定步数上限；每次滑杆操作合并成一步，可撤销、重做或直接跳到任意步骤。撤销后新编辑会替换后续分支。Ctrl/⌘ Z 撤销，Ctrl/⌘ Shift Z 或 Ctrl Y 重做。
- **预览与导出**：原图、滑动对比、放大预览，原图长边或 2048 / 1200 px PNG。导出捕获点击时的完整作品，继续编辑不会改变正在导出的内容。

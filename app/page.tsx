...
type Settings = { cell: number; coverage: number; lift: number; contrast: number; texture: number; fade: number; start: number; brightness: number; color: string; background: string };
const DEFAULTS: Settings = { cell: 4, coverage: 50, lift: 32, contrast: 105, texture: 90, fade: 92, start: 15, brightness: 100, color: '#8a76e9', background: '#050508' };
const PRESETS = [
  { name: '参考紫黑', caption: '规则网点 · 向下渐暗', values: DEFAULTS, className: 'reference' },
...
export default function Home() {
  ...
  useEffect(()=>{
    const worker=new Worker('/processor.js');workerRef.current=worker;

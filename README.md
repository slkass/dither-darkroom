...
- 三种预设：参考紫黑、保留细节、纯网点。
- 8 种网点结构：参考十字层次、旧版方点抖色、十字、交叉、圆点、方块、菱形和平行线。
- 调节大小、覆盖率、强度、旋转、暗部细节与细节增强；几何形状还可调整间距，十字、交叉、平行线可调整笔画粗细。
- 独立设置网点色、中间色和暗部色，以及亮度、对比度、渐暗方向/起终点、四周暗角和胶片颗粒。
- 查看原图、滑动对比和放大预览；对比模式可直接拖动画面或中央把手，支持触屏、下方滑块和方向键。
...
`npm test` 检查网点连接形态、几何形状、渐暗、缩放一致性和对比拖拽逻辑。
